  ...
  return (
    <div id="contract-preview" className="bg-white p-10 max-w-[210mm] mx-auto text-[13px] text-black leading-relaxed font-serif print:p-0">
      ...
      <div className="mb-4">
        ...
        <p><span className="font-semibold">3.3. Ca tối / Evening Shift:</span> {formData.eveningShift || formData.afternoonShift}</p>
      </div>
      ...
      <div className="mb-4">
        ...
        <p className="mb-3">{formData.probationWorkingTime ?? `${formData.workingDays} ${formData.morningShift} and ${formData.eveningShift || formData.afternoonShift}`}</p>
          ...
          <tbody>
            {(() => {
              ...
              const rows = [
                ...
                ["Phụ cấp Di chuyển / Transportation Allowance", formData.probationTransportNotApplicable ? "Không áp dụng / Not applicable" : formatAmount(trans1), formData.probationTransportNotApplicable ? "Không áp dụng / Not applicable" : formatAmount(trans2)],
                ...
                ["Phụ cấp PR / PR Allowance", formData.probationPrNotApplicable ? "Không áp dụng / Not applicable" : formatAmount(pr1), formData.probationPrNotApplicable ? "Không áp dụng / Not applicable" : formatAmount(pr2)],
                ["Phụ cấp Y tế / Medical Allowance", formatAmount(medical1), formatAmount(medical2)],
                ["Phụ cấp Chuyên cần / Reliability", formData.probationReliabilityNotApplicable ? "Không áp dụng / Not applicable" : formatAmount(reliability1), formData.probationReliabilityNotApplicable ? "Không áp dụng / Not applicable" : formatAmount(reliability2)],
