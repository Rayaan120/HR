...
import { AlertTriangle, BriefcaseBusiness, Building2, Download, Filter, LayoutGrid, MinusCircle, Plus, Printer, Search, SunMedium, Sunset, UserCheck, UserRoundSearch, Users, X } from "lucide-react";
import StaffProfileModal from "../components/StaffProfileModal";
import { exportToPDF, printDocument } from "../utils/pdfGenerator";
import { getDepartments, getJobPositions, getStaffProfiles, getWorkLocations, updateStaffProfile } from "../utils/storage";
...
const profileBranch = (profile) => String(profile.workLocation || profile.workLocation1 || profile.branch || "").split(";")[0].trim();
const profileShift = (profile) => profile.assignedShift || "";
const cellKey = (branch, shift, position) => `${branch}::${shift}::${position}`;
const noteKey = (branch, shift) => `${branch}::${shift}`;
const eligible = (profile) => profile.contractStatus === "Signed" && employmentStatus(profile) === "Active";
...
const toneFor = (required, assigned) => {
  ...
  if (assigned > required) return { key: "over", label: `Over by ${assigned - required}`, cell: "border-amber-200 bg-amber-50/70 text-amber-700", badge: "bg-amber-100 text-amber-700", dot: "bg-amber-400" };
  return { key: "full", label: "Fully staffed", cell: "border-emerald-200 bg-emerald-50/70 text-emerald-700", badge: "bg-emerald-100 text-emerald-700", dot: "bg-emerald-400" };
};
...
export default function StaffStatusReport() {
  ...
  const [filters, setFilters] = useState({ branch: "", position: "", department: "Kitchen Staff", shift: "", coverage: "", employee: "" });
  ...
  useEffect(() => {
    ...
    const all = () => { staff(); positions(); branches(); };
  ...
  const signedStaff = useMemo(() => profiles.filter(eligible), [profiles]);
  const departments = useMemo(() => [...new Set([...getDepartments(), ...jobs.map((j) => j.department), ...signedStaff.map((p) => p.department)].filter(Boolean))], [jobs, signedStaff]);
  const positions = useMemo(() => [...new Set([
    ...jobs.filter((j) => !filters.department || j.department === filters.department).map((j) => j.title),
    ...signedStaff.filter((p) => !filters.department || p.department === filters.department).map((p) => p.jobTitle),
  ].filter(Boolean))], [filters.department, jobs, signedStaff]);
  const branches = useMemo(() => [...new Set([...locations.map((l) => l.name), ...signedStaff.map(profileBranch)].filter(Boolean))], [locations, signedStaff]);
  const scheduled = signedStaff.filter((p) => profileBranch(p) && profileShift(p));
  const shownBranches = filters.branch ? branches.filter((b) => b === filters.branch) : branches;
  const shownPositions = filters.position ? positions.filter((p) => p === filters.position) : positions;
  const shownShifts = filters.shift ? SHIFTS.filter((s) => s === filters.shift) : SHIFTS;

  const assigned = (branch, shift, position) => scheduled.filter((p) => profileBranch(p) === branch && profileShift(p) === shift && p.jobTitle === position);
  ...
  const visibleRows = (branch) => shownShifts.filter((shift) => !filters.coverage || rowTone(branch, shift) === filters.coverage);
  ...
  const openRemove = () => { setDialog({ mode: "remove", branch: filters.branch, shift: filters.shift, position: filters.position }); setDialogSearch(""); };
  const assignPerson = (person) => {
    if (profileShift(person)) return alert(`${person.fullName} is already assigned to ${profileShift(person)} shift.`);
    updateStaffProfile(person.employeeId, { assignedShift: dialog.shift, statusReportBranch: dialog.branch, workLocation: dialog.branch, workLocation1: dialog.branch });
    setDialog(null);
  };
  const removePerson = (person) => updateStaffProfile(person.employeeId, { assignedShift: "", statusReportBranch: "" });
  const updateSelected = (person) => { updateStaffProfile(person.employeeId, person); setSelectedProfile(person); };

  const dialogPeople = signedStaff.filter((person) => {
    if (dialog?.mode === "assign" && (profileShift(person) || person.jobTitle !== dialog.position)) return false;
    if (dialog?.mode === "remove") {
      if (!profileShift(person)) return false;
      if (dialog.branch && profileBranch(person) !== dialog.branch) return false;
      if (dialog.shift && profileShift(person) !== dialog.shift) return false;
  ...
  const cards = [
    ["Total Employees", signedStaff.length, Users, "violet"], ["Scheduled Today", new Set(scheduled.map((p) => p.employeeId)).size, UserCheck, "blue"],
  ...
  return <div className="mx-auto max-w-[1900px] pb-12">
    <header className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-5 bg-gradient-to-r from-slate-950 via-slate-900 to-violet-950 px-6 py-7 text-white lg:flex-row lg:items-center lg:justify-between lg:px-8">
        ...
        <div className="flex flex-wrap gap-2 no-print"><Action icon={Plus} label="Assign Employee" primary onClick={() => openAssign()} /><Action icon={MinusCircle} label="Remove" onClick={openRemove} /><Action icon={Download} label="PDF" onClick={() => exportToPDF("staff-status-report-print", "Staff_Status_Report.pdf")} /><Action icon={Printer} label="Print" onClick={() => printDocument("staff-status-report-print")} /></div>
      </div>
    </header>
    ...
    <div id="staff-status-report-print">
      ...
      <section className="no-print mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex items-center justify-between"><h2 className="flex items-center gap-2 font-extrabold text-slate-900"><Filter size={17} className="text-violet-600" /> Report Filters</h2><button type="button" onClick={() => setEditNeeds(!editNeeds)} className={editNeeds ? "btn-primary" : "btn-secondary"}>{editNeeds ? "Save Requirements" : "Edit Required Counts"}</button></div>
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7"><Select label="Branch" value={filters.branch} options={branches} all="All branches" change={(v) => setFilters({ ...filters, branch: v })} /><Select label="Position" value={filters.position} options={positions} all="All positions" change={(v) => setFilters({ ...filters, position: v })} /><Select label="Department" value={filters.department} options={departments} all="All departments" change={(v) => setFilters({ ...filters, department: v, position: "" })} /><Select label="Shift" value={filters.shift} options={SHIFTS} all="Morning & evening" change={(v) => setFilters({ ...filters, shift: v })} /><Select label="Coverage" value={filters.coverage} options={[{ value: "under", label: "Only understaffed" }, { value: "full", label: "Only fully staffed" }]} all="All coverage" change={(v) => setFilters({ ...filters, coverage: v })} /><label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-slate-500">Search Employee</span><span className="relative block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} /><input className="input-field pl-9" value={filters.employee} onChange={(e) => setFilters({ ...filters, employee: e.target.value })} placeholder="Name or employee ID" /></span></label></div>
      </section>

      <div className="mt-4 flex flex-wrap gap-3 text-xs font-bold"><Legend color="bg-emerald-400" text="Fully staffed" /><Legend color="bg-rose-400" text="Staff required" /><Legend color="bg-amber-400" text="Overstaffed" /><Legend color="bg-slate-300" text="No requirement" /><span className="ml-auto text-slate-400">Updated {new Date().toLocaleString()}</span></div>

      <section className="mt-4 hidden overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm lg:block"><div className="max-h-[68vh] overflow-auto"><table className="min-w-max border-separate border-spacing-0 text-left text-sm"><thead className="sticky top-0 z-30"><tr><th className="sticky left-0 z-40 min-w-44 border-b border-r border-slate-700 bg-slate-900 px-4 py-4 text-xs font-black uppercase tracking-wider text-white">Branch</th><th className="sticky left-44 z-40 min-w-28 border-b border-r border-slate-700 bg-slate-900 px-4 py-4 text-xs font-black uppercase tracking-wider text-white">Shift</th>{shownPositions.map((position) => {
        ...
        const rows = visibleRows(branch); return rows.map((shift, index) => <tr key={`${branch}-${shift}`}>{index === 0 && <th rowSpan={rows.length} className="sticky left-0 z-20 border-b border-r border-slate-200 bg-white px-4 py-5"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-violet-600"><Building2 size={18} /></span><span className="max-w-28 break-words font-black text-slate-900">{branch}</span></div></th>}<th className="sticky left-44 z-20 border-b border-r border-slate-200 bg-white px-4 py-5"><span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-black ${shift === "Morning" ? "bg-sky-50 text-sky-700" : "bg-indigo-50 text-indigo-700"}`}>{shift === "Morning" ? <SunMedium size={14} /> : <Sunset size={14} />}{shift}</span></th>{shownPositions.map((position) => {
          const people = assigned(branch, shift, position), r = required(branch, shift, position); return <StaffCell key={position} {...{ branch, shift, position, people, required: r, tone: toneFor(r, people.length), editNeeds, search: filters.employee, updateNeed, openAssign, removePerson, setSelectedProfile }} />;
      ...
      })}</tbody></table>{!shownPositions.length && <Empty text="No positions match the selected filters." />}{!shownBranches.length && <Empty text="No branches are configured in Contract Generator settings." />}</div></section>
    ...
    <AnimatePresence>{dialog && <AssignDialog dialog={dialog} setDialog={setDialog} branches={branches} positions={positions} people={dialogPeople} search={dialogSearch} setSearch={setDialogSearch} assign={assignPerson} remove={removePerson} />}</AnimatePresence>
